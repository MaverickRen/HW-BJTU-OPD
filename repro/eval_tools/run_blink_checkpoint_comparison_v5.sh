...
WORKSPACE=/minimax-3d-rw-backup/users/jiazhi/H_Workspace
PYTHON="$WORKSPACE/UV_Env/verl-opd-qwen35/bin/python"
VLLM="$WORKSPACE/UV_Env/verl-opd-qwen35/bin/vllm"
SCORER="$WORKSPACE/Codes/opd-qwen35/eval_tools/mcq_blink_checkpoint_comparison_aggregate_v5.py"
AUDITOR="$WORKSPACE/Codes/opd-qwen35/eval_tools/mcq_blink_format_audit_v1.py"
...
[[ "$(id -u):$(id -g)" == 30853:30853 ]] || die 'execute requires uid/gid 30853'
exec 8<"$LOCK"
/usr/bin/flock 8
...
# roots are not writable by uid 30853.  Bind every runtime cache explicitly so
...
TMP_ROOT="/tmp/opd-blink-v5-30853-$PORT"
...
/usr/bin/setsid env \
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 HF_HUB_OFFLINE=1 TOKENIZERS_PARALLELISM=false LOGNAME=opd_uid_30853 USER=opd_uid_30853 \
