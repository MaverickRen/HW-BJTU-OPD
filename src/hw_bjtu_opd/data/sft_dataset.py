...
class B54TenKSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | list[str], tokenizer: Any, config: Any, processor: Optional[Any] = None, max_samples: int = -1) -> None:
        ...
        self.parquet_roots = {Path(path).resolve().parent for path in self.parquet_files}
    ...
    def _resolve_image(self, value: Any) -> dict[str, Any]:
        """Resolve paths relative to the portable Parquet snapshot.

        The original research artifact used absolute workspace paths.  The
        public SFT_V1 snapshot rewrites them to ``media/...`` so a downloaded
        Hugging Face snapshot remains directly trainable.
        """

        image = dict(_as_list(value))
        for key in ("path", "image"):
            ...
            if not path.is_absolute():
                if len(self.parquet_roots) != 1:
                    raise ValueError("relative media paths require one Parquet root")
                path = next(iter(self.parquet_roots)) / path
            image[key] = str(path)
        return image
                    ...
                    content.append(
                        {
                            "type": "image",
                            "image": process_image(
                                self._resolve_image(images[image_index]),
